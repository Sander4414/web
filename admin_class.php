...
Class Action {
    ...
    if(!empty($_FILES['upload']['name'][0])){
        for($i=0; $i<count($_FILES['upload']['name']); $i++){
            $fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['upload']['name'][$i];
            $move = move_uploaded_file($_FILES['upload']['tmp_name'][$i],'assets/uploads/'. $fname); 
            if($move){
	...
	function save_compartir(){
		extract($_POST);
		$this->db->query(" DELETE FROM compartircarpeta where carpeta_id='$carpeta_id'");
	...
	function save_compartir_documento(){
		extract($_POST);
		foreach ($usuario_id as $user_id) {
			$data = " usuario_id = '$user_id' ";
